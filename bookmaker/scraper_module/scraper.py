    ...
    async def monitor_live_page_persistent(self, status_check_callback=None, log_callback=None):
        ...
        try:
            ...
            def get_db_live_identifiers():
                identifiers = set()
                # Use values_list to avoid loading Decimal fields that may contain bad data
                for match_id, match_url, home_name, away_name in Match.objects.filter(status='live').values_list(
                        'id', 'match_url', 'home_team__name', 'away_team__name'
                ):
                    if match_url:
                        identifiers.add(match_url)
                    else:
                        home = home_name.strip().lower()
                        away = away_name.strip().lower()
            ...
            while True:
                ...
                try:
                    ...
                    if scraped_data:
                        ...
                        for m in scraped_data:
                            ...
                            for odd_type in ['home_odds', 'draw_odds', 'away_odds']:
                                m[odd_type] = self.parse_odds(m.get(odd_type))
                        ...
                        for m in scraped_data:
                            display_id = m.get('match_url') or f"{m['home_team'].strip().lower()}|{m['away_team'].strip().lower()}"
                            ...
                            for identifier in disappeared_matches:
                                ...
                                if hasattr(self, 'mark_match_as_finished_by_identifier'):
                                    try:
                                        ...
                                        await log(f"      ⚠️ Failed to finish match {identifier}: {e}")
                        ...
                        if db_live_identifiers:
                            if iteration_count <= 2:
                                ...
                                await log(
                                    f"🚨 Page empty. Marking all {len(db_live_identifiers)} DB matches as Finished.")
                    ...
                    await log(f"❌ Error in monitor iteration: {e}")
                        ...
                        if match_data.get('home_odds') and match_data.get('away_odds'):
                            ...
                            if match.home_odds and match.away_odds:
                                try:
                                    ...
                                    print(f"Warning: Failed derived odds for {match.home_team} vs {match.away_team}: {calc_error}")
                except Exception as e:
                    print(f"Error saving live match {match_data.get('home_team')} vs {match_data.get('away_team')}: {e}")
                    continue
            return saved_count
